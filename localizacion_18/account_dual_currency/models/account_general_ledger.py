    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        for account, column_group_results in self._query_values(report, options):
            ...
            for column_group_key, results in column_group_results.items():
                ...
                account_debit = (account_sum.get('debit') or 0.0) + (account_un_earn.get('debit') or 0.0)

                account_credit = (account_sum.get('credit') or 0.0) + (account_un_earn.get('credit') or 0.0)

                account_balance = (account_sum.get('balance') or 0.0) + (account_un_earn.get('balance') or 0.0)
